...
2. 通过`vars`设置ansible部署过程中的环境变量，ansible脚本中通过双大括号方式引用；
...
6. 通过`copy`模块上传文件到部署服务器，与`template`的区别在于`template`上传文件时，会对文件中的双大括号方式引用的参数进行自动填充；
...
# 2. {% for host in groups.mongodb -%}和{% endfor %}为for循环语句，即循环mongodb组中所有的主机host信息；
